...
// session 을 사용하기 위한 설정
// 반드시 cookieParser() 설정 이후에 위치
// app.use() 미들웨어 설정하는 함수
app.use(
  session({
    key: "tory", //식별자, 브라우저에 저장될 cookie 이름, 프로젝트 이름으로 많이씀
    secret: "wjdduscldrn@naver.com", // SessionID 암호화용 키
    cookie: {
      httpOnly: true,
      maxAge: 1000 * 60 * 60, // 유효시간, 60분동안 유지하고 삭제해
    },
  })
);
// 로그인을 하지 않아도 될 곳(회원 인증)
app.use("/users", usersRouter);

/**
 * 모든 요청 http://localhost:3000/* 로 요청한 것들, 모든것에 대한 라우터
...
app.use((req, res, next) => {
  res.locals = req.session; // locals 에 session 정보 저장하기
  next(); // 다음 라우터로 가렴
});
