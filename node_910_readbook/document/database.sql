...
정수값 : TINYINT, SMALLINT, INT(INTEGER), BIGINT
	...
	SMALLINT : 0 ~ 65535 ( 또는 -32768 ~ 32767)
...
-- INSERT 해 보기
INSERT INTO tbl_books(isbn,title,author,publisher) VALUES('0001','자바','노도강','자바출판사');
SELECT * FROM tbl_books;

/*
Workbench, 프로젝트, 다양한 DB client(db서버와 통신하는 친구) 에서 데이터를 INSERT, UPDATE, DELETE 를 실행하면
성능상 여러 이유로 직접 DB 저장소에 바로 데이터가 저장되지 않고
중간에 임시 기억 장치에 잠시 머물게 된다.
일부 다른 DB 소프트웨어는 임시 기억 장소에 아예 데이터를 저장해두고, 어떤 명령등이 실행되면
한꺼번에 데이터를 저장소에 저장하기도 한다.

이러한 중간의 임시장치를 BUFFER 또는 cache 라고 한다.
이 때 여러곳의 client 가 동시에 DB 의 데이터에 접근할 경우 일부 client 에서는
...
-- 강제로 저장소에 저장해라
COMMIT; 
INSERT INTO tbl_books(isbn,title,publishe,author) VALUES('0002','HTML','한국출판사','디자이너');

-- INSERT, UPDATE, DELETE 명령을 취소해라
-- INSERT 하고 아무 명령도 하지않고 ROLLBACK 하면 취소된다
ROLLBACK;
SELECT * FROM tbl_books;

/*
DDL(Date Definition Lang.) : DBA (Data Administor, 최고관리자) 가 사용하는 명령어
물리적인 저장소 생성: CREATE DATABASE, CREATE TABLE
사용자를 생성: CREATE USER
물리적인 저장소 삭제: DROP DATABASE, DROP TABLE
*/

-- DB 에서 root 사용자는 매우 신중하게 사용해야하는 권한을 가지고 있다.
-- DB 의 물리적 저장소를 생성, 제거, 변경 할수 있는 권한을 갖는다.
-- 일반적으로 현업에서는 root 사용자 외의 별도의 사용자를 생성하여
-- 제한적인 권한을 부여하여 사용하도록 한다.

-- yangtory 라는 사용자를 localhost 에서만 접근할수 있도록 생성한다.
...
무결성을 보장하는 방법 : 생성된 사용자에게 적절한 권한을 제한적으로 부여하여
	여러 민감한 데이터의 손상, 변화(INSERT, UPDATE, DELETE)를 최소화 하여야 한다.
    무결성은 내가 한 번 인서트한 데이터를 업뎃, 델리트 하지 않았으면 항상 그대로 유지된다는 말임
*/

-- yangtory 사용자에게 bookdb2 데이터베이스에 대하여 권한을 부여하기
